...
def main(train_path, eval_path, save_path):
    ...
    init = PoissonRegression(step_size=1e-5, max_iter=10000, eps=1e-5,
    ...
    pred = np.zeros((1, N_p))
    pred = np.matmul(theta_final.T,x_eval)
    xx = np.zeros((1,N_p))
...
class PoissonRegression:
    ...
    def __init__(self, step_size=1e-5, max_iter=10000, eps=1e-5,
                 theta_0=None, verbose=True, lr = 1):
        """
        Args:
    ...
    def fit(self, x, y):
        ...
        while np.max(difference) > self.eps and iteration < self.max_iter :
            iteration += 1
            print(iteration)
            h = PoissonRegression.predict(self,x)
